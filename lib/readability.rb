...
module Readability
  class Document
    ...
    def initialize(input, options = {})
      ...
      # User customize
      if @input.encoding.to_s == 'ASCII-8BIT'
        @input = @input.gsub(REGEXES[:replaceBrsRe], '</p><p>').gsub(REGEXES[:replaceFontsRe], '<\1span>')
      else
        @input = @input.encode('UTF-16BE', :invalid => :replace, :undef => :replace, :replace => '?').encode('UTF-8').gsub(REGEXES[:replaceBrsRe], '</p><p>').gsub(REGEXES[:replaceFontsRe], '<\1span>')
      end
      # /User customize
